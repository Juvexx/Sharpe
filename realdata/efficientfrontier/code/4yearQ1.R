...
for (i in 0:125){
  ...
  for (j in 1:30){
    ...
    # Resolvent_M=solve(hSigma+qlist[j]*Q0)

    # Rvecmu=Resolvent_M%*%mu
    # Rvec1=Resolvent_M%*%one
    A=(t(mu)%*%Rvec1)[1]
    B=(t(mu)%*%Rvecmu)[1]
    C=(t(one)%*%Rvec1)[1]
    D=B*C-A^2
...
write.table(Result_day,paste('meanvariance/4yearQ1/resultdaymu0=',as.character(mu0),'.txt',sep=''),row.names = FALSE)
write.table(cbind(Rolling_time_month,q_id_month),paste('meanvariance/4yearQ1/q_id_monthmu0=',as.character(mu0),'.txt',sep=''),row.names = FALSE)
write.table(cbind(Rolling_time_month,q_diag_month),paste('meanvariance/4yearQ1/q_diag_monthmu0=',as.character(mu0),'.txt',sep=''),row.names = FALSE)
...
listsd=NULL
for (mu0 in mu0list){
global<-read.table(paste('meanvariance/4yearQ1/resultdaymu0=',as.character(mu0),'.txt',sep=''),head=TRUE)
...
write.csv(listsd,'meanvariance/4yearQ1/listsd.csv')
