...
for (i in 0:125){
  ...
  id1=id1[,-1]*1.0
  Q2=sqrt(diag(apply(id1,2,mean)/1e8))
  ...
  for (j in 1:30){
    ...
    listEstSR=c(listEstSR, adjustfactor*(t(w)%*%mu-(1-adjustfactor)/adjustfactor)/sqrt(t(w)%*%hSigma%*%w))

    w0=w/sum(abs(w))
    # t(w)%*%mu/sqrt(t(w)%*%hSigma%*%w)
    if (j==1){Result=X_test%*%w0-Factor_data_test[,5]}
    else {
       Result=cbind(Result,X_test%*%w0-Factor_data_test[,5])
    }
  }
...
write.table(Result_day,'meanvarianceunknown/year1/resultday1.txt',row.names = FALSE)
write.table(cbind(Rolling_time_month,q_id_month),'meanvarianceunknown/year1/q1_id_month.txt',row.names = FALSE)
write.table(cbind(Rolling_time_month,q_diag_month),'meanvarianceunknown/year1/q1_diag_month.txt',row.names = FALSE)

 #########3yearsSR
global<-read.table('meanvarianceunknown/year1/resultday1.txt',head=TRUE)
...
write.csv(listsd,'meanvarianceunknown/year1/listSRQ2.csv')