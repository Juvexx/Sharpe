...
for repeat0 in range(1000):
    ...
    listlambda,SRknown,SRunknown=getmean(pathsmall+'1500repeat'+str(repeat0+1)+'.csv')
...
ax1.legend(loc='lower right', ncol=1,
        fancybox=True)
plt.savefig('figures/smallbase.pdf',bbox_inches='tight')
...
for repeat0 in range(1000):
    ...
    listlambda,SRknown,SRunknown=getmean(pathbig+'1500repeat'+str(repeat0+1)+'.csv')
    listSRknown=listSRknown+np.array(SRknown)
    listSRunknown=listSRunknown+np.array(SRunknown)
    known=listSRknown/1000
    unknown=listSRunknown/1000
...
plt.savefig('figures/bigbase.pdf',bbox_inches='tight')
